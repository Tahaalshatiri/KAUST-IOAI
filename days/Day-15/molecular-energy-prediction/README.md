# Molecular Energy Prediction

**Kaggle competition link:** _TODO: add link_

**Data:** not stored in git — see `molecular-energy-prediction.zip` in the shared Google Drive
folder (link in the main README), or the Kaggle page above.

_TODO: short description of the competition (task, data, metric)._

## Baseline

`baseline.ipynb` â€” per-molecule element counts + charge/geometry aggregates, LightGBM. CV MAE: **66.0 eV**.
